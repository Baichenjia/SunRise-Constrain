...
# 多了一个 mask 作为输入，用来指定哪些样本用于给哪些 ensemble 元素训练
Transition = namedtuple('Transition', ('timestep', 'state', 'action', 'reward', 'nonterminal', 'mask', 'onlineQ'))
...
class ReplayMemory:
    def __init__(self, args, capacity, beta_mean, num_ensemble, action_space):
        ...
        self.blank_trans = Transition(0, torch.zeros(84, 84, dtype=torch.uint8), None, 0, False,
                             torch.zeros(5, dtype=torch.float32), torch.zeros(self.num_ensemble, action_space, 51, dtype=torch.float32))

    # Adds state and action at time t, reward and terminal at time t + 1
    def append(self, state, action, reward, terminal, onlineQ):
        ...
        self.transitions.append(Transition(self.t, state, action, reward, not terminal, mask, onlineQ), self.transitions.max)  # Store new transition with maximum priority
    ...
    def _get_sample_from_segment(self, segment, i):
        ...
        ###########################
        onlineQ = transition[self.history - 1].onlineQ.detach().to(device=self.device)

        return prob, idx, tree_idx, state, action, R, next_state, nonterminal, mask, onlineQ

    def sample(self, batch_size):
        p_total = self.transitions.total()  # Retrieve sum of all priorities (used to create a normalised probability distribution)
        segment = p_total / batch_size  # Batch size number of segments, based on sum over all probabilities
        batch = [self._get_sample_from_segment(segment, i) for i in range(batch_size)]  # Get batch of valid samples
        probs, idxs, tree_idxs, states, actions, returns, next_states, nonterminals, masks, onlineQ = zip(*batch)
        ...
        onlineQ = torch.stack(onlineQ)
        return tree_idxs, states, actions, returns, next_states, nonterminals, weights, masks, onlineQ
    ...
    # Alias next for Python 2 compatibility
    next = __next__
