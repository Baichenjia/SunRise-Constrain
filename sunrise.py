...
from mmd_loss import calculate_mmd, calculate_kl

seed = np.random.randint(12345)

# Note that hyper-parameters may originally be reported in ATARI game frames instead of agent steps
...
parser.add_argument('--memory-capacity', type=int, default=int(5e5), metavar='CAPACITY', help='Experience replay memory capacity')  # 此处与CURL不同，CURL为1e5
...
parser.add_argument('--beta-mean', type=float, default=1.0, help='mean of bernoulli')     # 1.0
parser.add_argument('--temperature', type=float, default=10.0, help='temperature for CF')  # 10,40. Corrective feedback
parser.add_argument('--ucb-infer', type=float, default=1.0, help='coeff for UCB infer')   # 1, 10
...
def run_main():
    ...
    # exp name
    exp_name = args.id + '_' + args.game
    # exp_name += '/Beta_' + str(args.beta_mean) + '_T_' + str(args.temperature)
    # exp_name += '_UCB_' + str(args.ucb_infer) + '/'
    ...
    if args.model is not None and not args.evaluate:
        ...
        mem = ReplayMemory(args, args.memory_capacity, args.beta_mean, args.num_ensemble, action_space)

    priority_weight_increase = (1 - args.priority_weight) / (args.T_max - args.learn_start)

    # Construct validation memory
    val_mem = ReplayMemory(args, args.evaluation_size, args.beta_mean, args.num_ensemble, action_space)
    T, done = 0, True
    while T < args.evaluation_size:
        if done:
            state, done = env.reset(), False
        next_state, _, done = env.step(np.random.randint(0, action_space))
        val_mem.append(state, None, None, done, torch.zeros(args.num_ensemble, action_space, 51, dtype=torch.float32))
        state = next_state
        T += 1
    ...
    if args.evaluate:
        ...
        for T in trange(1, args.T_max + 1):
            ...
            # UCB exploration
            onlineQ_stack = []  # new add ###
            if args.ucb_infer > 0:
                mean_Q, var_Q = None, None
                L_target_Q = []
                for en_index in range(args.num_ensemble):
                    onlineQ_stack.append(dqn_list[en_index].online_net(state.unsqueeze(0)).cpu())
                    target_Q = dqn_list[en_index].get_online_q(state)  # (None, n_action)
                    L_target_Q.append(target_Q)
                    if en_index == 0:
                        mean_Q = target_Q / args.num_ensemble
                    else:
                        mean_Q += target_Q / args.num_ensemble   # mean Q
                temp_count = 0
                onlineQ_stack = torch.squeeze(torch.stack(onlineQ_stack, dim=1), dim=0)  # (3, n_action, 51). 5 是 ensemble 数目
                ...
                std_Q = torch.sqrt(var_Q).detach()               # std of Q
                ucb_score = mean_Q + args.ucb_infer * std_Q
                action = ucb_score.argmax(1)[0].item()           # ucb 选择的动作
            else:
                action = dqn_list[selected_en_index].act(state)  # Choose an action greedily (with noisy weights)
            next_state, reward, done = env.step(action)          # Step
            if args.reward_clip > 0:
                reward = max(min(reward, args.reward_clip), -args.reward_clip)  # Clip rewards
            mem.append(state, action, reward, done, onlineQ_stack.clone().detach())   # Append transition to memory
            ...
            if T >= args.learn_start:
                ...
                if T % 50 == 0:
                    batch_mmd = calculate_mmd(mem, dqn_list, round=10, batch_size=64)
                    batch_kl = calculate_kl(mem, dqn_list, round=10, batch_size=64)
                    print("learning start")
                    print("steps:", T, ", MMD:", batch_mmd, ", KL:", batch_kl)
                    with open("mmd_notrain.txt", 'a') as f:
                        f.write(str(T)+","+str(batch_mmd)+","+str(batch_kl)+"\n")
                ...
                if T % args.replay_frequency == 0:
                    ...
                    # Sample transitions
                    idxs, states, actions, returns, next_states, nonterminals, weights, masks, onlineQ = mem.sample(args.batch_size)
                    q_loss_tot = 0
